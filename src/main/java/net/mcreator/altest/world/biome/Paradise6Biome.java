...
import net.mcreator.altest.block.AutumnleavesBlock;
import net.mcreator.altest.AltestModElements;

import com.google.common.collect.ImmutableList;

@AltestModElements.ModElement.Tag
public class Paradise6Biome extends AltestModElements.ModElement {
	public static Biome biome;

	public Paradise6Biome(AltestModElements instance) {
		super(instance, 441);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-2062764).setWaterColor(-16763166).setWaterFogColor(329011)
						.withSkyColor(-2062764).withFoliageColor(10387789).withGrassColor(-2454475).build();
				...
				biomeGenerationSettings.withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.TREE
						.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(Blocks.BIRCH_LOG.getDefaultState()),
								new SimpleBlockStateProvider(AutumnleavesBlock.block.getDefaultState()),
				...
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(EntityType.LLAMA, 1, 1, 2));
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.7999999999999999f).scale(1.1f)
						.temperature(0.7f).downfall(0.2f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("altest:paradise_6"));
			}
		}
	}
