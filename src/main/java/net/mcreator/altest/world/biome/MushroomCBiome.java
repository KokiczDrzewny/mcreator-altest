...
public class MushroomCBiome extends AltestModElements.ModElement {
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-6736897).setWaterColor(-6750055).setWaterFogColor(-10092442)
						.withSkyColor(-6736897).withFoliageColor(-13421569).withGrassColor(-3407719).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.MUSHROOM).depth(0.2f).scale(0.3f)
						.temperature(0.7f).downfall(0.7f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("altest:mushroom_c"));
			}
		}
	}
