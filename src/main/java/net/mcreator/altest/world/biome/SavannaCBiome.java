...
public class SavannaCBiome extends AltestModElements.ModElement {
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-3355648).setWaterColor(-16738048).setWaterFogColor(-16738048)
						.withSkyColor(-3355648).withFoliageColor(-13382656).withGrassColor(-10066432).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.SAVANNA).depth(0.2f).scale(0.2f)
						.temperature(1.2000000000000002f).downfall(0.1f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("altest:savanna_c"));
			}
		}
	}
