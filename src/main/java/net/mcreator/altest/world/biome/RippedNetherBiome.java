...
public class RippedNetherBiome extends AltestModElements.ModElement {
	public static Biome biome;

	public RippedNetherBiome(AltestModElements instance) {
		super(instance, 643);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(-65536).setWaterColor(-6737152).setWaterFogColor(329011)
						.withSkyColor(-65536).withFoliageColor(10387789).withGrassColor(-6750157).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(Blocks.CRIMSON_NYLIUM.getDefaultState(),
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.NONE).category(Biome.Category.NETHER).depth(0.6f).scale(0.7f).temperature(2f)
						.downfall(0f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("altest:ripped_nether"));
			}
		}
	}
